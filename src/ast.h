...
typedef enum {
  ...
  ND_CAST,
  ND_COND,
  // ND_STATEMENT corresponds to "Statement Expression" in GCC
  // while that is not supported in ccc,
  // ND_STATEMENT is required to achieve the compound assignments
  ND_STATEMENT,
} ExprKind;
...
struct Expr {
  ...
  BlockItemList* items;  // for ND_STATEMENT, owned
...
Expr* new_node_statement(BlockItemList*);
